...
class Model_Train_Test:
    ...
    def train_model_crossval(
        self, dataset: torch.utils.data.Dataset, k_folds: int = 10, epochs: int = 10, batch_size: int = 256, DP: int = 3
    ):
        score_df = []
        ...
        for fold, (train_idx, test_idx) in enumerate(kfold.split(dataset, dataset.labels)):
            ...
            for epoch in range(epochs):
                ...
                train_metrics, test_metrics = My_Pytorch_Utilities.metics_tensor_dict_to_floats(
                    train_metrics
                ), My_Pytorch_Utilities.metics_tensor_dict_to_floats(
                    test_metrics
                )  # Convert the dictionary with tensors on GPU to a dictionary with floats on CPU

            print(
                f"Fold {fold+1} final results after {epoch+1} epochs: Train Acc: {train_metrics['train_BinaryAccuracy']:.{DP}f} Train Loss: {train_loss:.{DP}f} (n = {len(train_idx)}) | Test Acc: {test_metrics['test_BinaryAccuracy']:.{DP}f} Test Loss: {test_loss:.{DP}f} (n = {len(test_idx)}) "
            )
    ...
    def train_model(self, dataset, train_size: float = 0.8, epochs: int = 10, batch_size: int = 256, DP: int = 3):
        ...
        for epoch in range(epochs):
            ...
            print(
                f"Results after {epoch+1} epochs: Train Acc: {train_metrics['train_BinaryAccuracy']:.{DP}f} Train Loss: {train_loss:.{DP}f} (n = {len(train_data)}) | Test Acc: {test_metrics['test_BinaryAccuracy']:.{DP}f} Test Loss: {test_loss:.{DP}f} (n = {len(validation_data)}) "
            )
